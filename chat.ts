...
export const chatToAssistantInternal = internalAction({
    args: {
        ...
        send_message: v.string(),
        thread:v.optional(v.string())
    }, handler: async (ctx, args): Promise<any> => chat.chatToAssistant({ctx, args: {
        user: (await ctx.runQuery(internal.users.internalGetUserByID, {user:args.user}))!,
        model_id: args.model_id,
        send_message: args.send_message,
        thread: args.thread
    }})
...
export const internalSaveChatToAI = internalMutation({
    ...
    handler: async (ctx, args): Promise<boolean> => {
        ...
        const history = await ctx.db.query('chat_history').filter(
            (q) => q.and(
