...
export default class Chat {
    ...
    public getMessageList = async (
        params: ListRequest,
    ): Promise<MessageResult[]> => this.messageService.getList(params);

    public chatToAssistant = async ({
        ctx,
        args,
    }: {
        ...
        const model = await ctx.runQuery(internal.openai_model.getModelById, {
            model: args.model_id,
        });
        ...
        const assistant = await ctx.runQuery(
            internal.openai_assistant.getAssistantByModel,
            { model: args.model_id },
        );
        if (!assistant) {
            const new_assistant = await this.assistantService.create({
                model: model!.id,
            });
            chatAssistant.isNew = true;
            chatAssistant.data =
                new_assistant as unknown as Doc<'openai_assistant'>;
    ...
    // 删除线程
    public deleteThread = async (thread: string) =>
        this.threadService.delete(thread);

    // 创建聊天 并 Run
    private createRun = async ({
        ctx,
        args,
        thread,
        assistant,
