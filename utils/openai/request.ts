...
export default class Request {
    ...
    constructor(apikey?:string | undefined) {
        if (apikey) {
            ...
            if (!process.env.OPENAI_API_KEY) {
                throw new ConvexError("You need to provide OPENAI_API_KEY env variable");
            }
    ...
    protected toUrlEncoded(obj: any): string {
        return Object.keys(obj)
            ...
            .map(key => `${encodeURIComponent(key)}=${encodeURIComponent(obj[key as keyof any]!)}`)
            .join('&');
    }

    // eslint-disable-next-line class-methods-use-this
    protected request = async (url: string, params: object) => fetch(url, params).then(response => {
        if (!response.ok) {
