...
export const getChatHistory = query({
    ...
    handler: async (ctx, args) => {
        ...
        return ctx.db.query('chat_history').filter(
            (q) => q.eq(q.field('user'), user._id)
...
export const getListAccordingToTimePeriod = query({
    ...
    handler: async (ctx, args) => {
        const user = await queryUser(ctx);
        const data = await ctx.db.query('chat_history').withIndex('by_update_time', (q) =>
            q
...
export const deleteHistory = action({
    ...
    handler: async (ctx, args) => {
        const user = await ctx.runQuery(internal.users.internalGetUserByIdentity);

        const history = await ctx.runQuery(internal.chat_history.internalGetChatHistoryByID, {history: args.history});
        if (!history) {
        ...
        try {
            ...
            await ctx.runMutation(internal.chat_history.internalDeleteHistory, args);
        } catch (error: any) {
...
export const internalDeleteHistory = internalMutation({
    args: { history: v.id('chat_history'), thread: v.string() },
    handler: async(ctx, args) => {
        const details = await ctx.db.query('chat_detail').filter(
            (q) => q.eq(q.field('thread'), args.thread),
        ).collect();
        await Promise.all(
